...
### 2.2 ★★ 滑动实测：每帧 IRQ 从 **15.39 降到 3.56**、CPU 占用降 63%

> **本节第一版写错了，特此更正并保留痕迹**：最初根据**空闲**数据判定"GSI 不会改变
> `irq_per_frame`，因为那条 IRQ 属于触屏 IC、与 SPI 模式无关"。**真实滑动实测推翻了它** ——
> 空闲时两版确实一样，滑动时差 **4.3 倍**。教训：**空闲数据不足以推断负载下的行为**。

**滑动对照**（同一台机器、同一支脚本 `tools/gk-touch-bench.py`，用户匀速来回划屏）：

| 指标 | r0（FIFO） | **r1（GSI）** | 变化 |
|---|---|---|---|
| 帧数 | 1,413 | **4,017** | +184% |
| 有效帧率（剔除 >100 ms 停顿） | 当时未记录该字段 | **119.3 Hz** | — |
| 帧间隔 中位 | 8.328 ms | 8.329 ms | 持平 |
| 帧间隔 p95 | 9.38 ms | 9.523 ms | 持平 |
| 帧间隔 p99 | **32.8 ms** | **10.267 ms** | **−69%**（抖动显著收窄） |
| **每帧触屏 IRQ** | **15.39** | **3.558** | **−77%** |
| 每帧触点事件 | 3.99 | 3.95 | 持平 |
| **CPU 忙** | **14.97 %** | **5.56 %** | **−63%** |

⇒ **报点率与帧间隔中位数不变（由 IC 决定），但每帧中断数 −77%、CPU 占用 −63%、p99 抖动 −69%**
（上表为工具**原始读数**）。折算到"每次触点事件"：r0 ≈ 3.9 次中断，r1 ≈ 0.9 次 —— 已接近
"IC 每报一次只触发一次"的物理下限。

> **口径提示（重要）**：r0 那次的 45 s 窗口内含一段约 29 s 的松手停顿；空闲时触屏 IC 仍以
> 约 120 次/秒触发中断（"幽灵中断"），故 r0 的 `irq_per_frame` 被**高估**、`cpu_busy_pct` 被**低估**。
> 扣除空闲段后更公平的估算是：每帧中断 **≈12.9 → ≈3.33（−74%）**；r1 滑动段内 CPU ≈ **6.6%**，
> 而 r0 的真实滑动段占用**高于**其窗口均值 14.97%，即"CPU 至少降到 1/2.7"。
> 原始 JSON、中断线增量与估算过程见 [`docs/bench/touch-bench-20260913.md`](../bench/touch-bench-20260913.md)。

**滑动期间 GSI 依然生效**（同一窗口的中断线增量）：`998000.spi`（GENI SE）**前后均为 0**，
`gpi-dma` +28,706，`msmgpio 175`（触屏 IC）+14,302，即约 2 次 DMA 中断 / 次触屏中断。

**为什么"空闲时两版一致、滑动时差 4 倍"**【机制为推测，现象为实测】：空闲时没有触点数据，
两版都只是 IC 约 120 次/秒的空转中断；滑动时 IC 持续产出数据，而 **FIFO 模式每次 SPI 读都要
逐字中断、排空慢，level-low 的 INT 在数据被读完前会反复触发**；GSI 用 DMA 快速排空，
于是每帧只需少数几次中断。
...
| # | 项 | 状态 |
|:-:|----|------|
| 1 | **滑动状态下的报点率 / 帧间隔 / 中断 / CPU 对照**（r0 vs r1） | ✅ **已测**（2026-09-13，用户匀速划屏）：每帧 IRQ 15.39 → **3.558**、CPU 14.97% → **5.56%**、p99 32.8 → **10.27 ms**，报点率与中位帧间隔持平。详见 §2.2 |
| 2 | GSI 对 CPU 占用与延迟的实际影响 | ✅ **已量化**：CPU 忙 −63%；p99 抖动 −69% |
| 3 | "空闲时 IC 仍以 ~120 次/秒中断却无输入事件"（幽灵中断） | **未查清**（r0/r1 皆有，非本版引入） |
| 4 | Chromium `V4L2VideoDecoder` 复核 | 未做 |
| 5 | 热管理（75 °C 节流）、音频 UCM | 仍未纳入本内核配套 |

> 测量口径：`tools/gk-touch-bench.py --seconds 45 --wait-timeout 900`，从**检测到首个触摸帧**
> 才开始计时；r0 那次的 `--seconds 45` 窗口内含一段约 29 s 的松手停顿（已由 `long_gaps`/`max` 反映，
> 故 r0 的 `rate_hz` 不具可比性，仅比较中位数与分位数）。
