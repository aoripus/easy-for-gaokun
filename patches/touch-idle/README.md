...
>
> - **用途**：修复 Himax HX83121A SPI 驱动的**中断使能不对称**缺陷，并加入**可选**的空闲门控采样策略
> - **基线 / Base**：**不是 mainline** —— 基线是"社区 HX83121A 驱动 **+** 本项目 r2 增加的 AFE 命令邮箱层"，
>   因此必须在本项目 gaokun3 补丁集**之后**应用（详见下面「补丁基线」）
> - **序列 / Order**：见同目录 [`series`](series)（单条补丁）
> - **上游状态**：未投稿。建议**拆成两个提交**分别上游化（缺陷修复 / 空闲策略），见下「上游化建议」
> - **是否用于当前发布内核**：✅ 是（自 r3 起）
> - **验证**：对 r1/r2 出厂驱动副本 `patch -p1 --dry-run` **22/22 hunk 全部干净应用**；
>   实机收益见下「实测收益」
> - **回滚**：策略可用 `idle_enter_frames=0` 关闭；移除补丁则回到"空闲时 120 Hz 中断"的行为
